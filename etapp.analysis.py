...
#mass in GeV
mp = 0.938272 #proton mass which is a byron
me = 0.95778 #eta prime meson mass
...
plt.title(r'$\gamma  p \rightarrow \eta^\prime p$', size =30)
color = 'tab:red'
...
for i in cosines:
    ...
    plt.title(r'$\gamma p \rightarrow \eta^\prime p$', size=35)
...
PDF = PdfPages('allcutsetapetapp.pdf')
...
for j in alpha[:-5]:
    ...
    plt.title(r'$\gamma  p \rightarrow \eta^\prime p$: $\frac{d\sigma}{dt}$=$(A + B \cos \Theta)s^{N \pm \delta N}$, $p_{T _{min}} ^2 = %2.2f}$, $\chi ^2 /df = %2.2f$' %(j,redchi)  , size = 25)
    # axis labels
